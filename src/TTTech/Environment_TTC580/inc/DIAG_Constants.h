 ...
 *
 * If a fatal error occurs, the diagnostic state machine notifies the
 * application about the error occurrence by calling this callback
 * function. If the watchdog CPU is configured with resets, a reset
 * is expected after 22ms.
 *
 * The callback function is passed to the function IO_Driver_Init()
 * in a field of the safety configuration (#IO_DRIVER_SAFETY_CONF).
 *
