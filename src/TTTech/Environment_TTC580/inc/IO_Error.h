 ...
 *  When writing: The data has not been accepted by the driver. The application needs to transmit
 *  it when there is enough space in the buffer.
 */
 ...
 *  The hardware buffer reported an overflow. This happens if to much data has been received
 *  between two consecutive SW cycles.
 */
...
/** The common driver init function \c IO_Driver_Init() has not been called, or reported the error
 *  \c #IO_E_FPGA_TIMEOUT.
 */
#define IO_E_FPGA_NOT_INITIALIZED           200U

/** The FPGA did not respond in time. Some ECU functions will not be available.
 */
...
/** The FPGA version does not match with this IO Driver Version. Some ECU functions will not be
 * available.
 */
